...
async def receive_user_data(
    ...
    try:
        ...
        if existing_user:
            ...
            #     )
            # Usuário existe mas não validado - criar NOVO token
            logger.info(f"Usuário {user_data.email} existe mas não validado - criando novo token")
            
            # Criar novo token com session_id
            ...
            return UserCreateResponse(
                success=True,
                message="Novo token de validação enviado por email",
                user_id=existing_user.id
            )
